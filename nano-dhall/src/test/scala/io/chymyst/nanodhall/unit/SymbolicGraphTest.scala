...
class SymbolicGraphTest extends FunSuite {
  ...
  test("circular dependencies do not create an infinite loop 1") {
    ...
    expect(b == new RuleDef(name = "b", rule =   Or(And(LiteralMatch("y"),   GrammarSymbol("b", () => b)), LiteralMatch("z"))))
  }
  ...
  test("circular dependencies do not create an infinite loop 2") {
    ...
    expect(
      b == new RuleDef(
        name = "b",
        rule =  Or(And(And(LiteralMatch("x"),   GrammarSymbol("a", () => a)),   GrammarSymbol("b", () => b)), LiteralMatch("y")),
      )
    )
