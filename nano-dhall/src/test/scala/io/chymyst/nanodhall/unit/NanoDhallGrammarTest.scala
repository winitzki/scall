...
class NanoDhallGrammarTest extends FunSuite {
  ...
  test("parse lambda") {
    expect(parser1.parse("\\(x : Natural) -> x + 1") == Lambda(VarName("x"), Constant(NanoConstant.Natural), Operator(Variable(VarName("x"), 0), NanoOperator.Plus, NaturalLiteral(1))))
  }


  test("parse lambda and applications") {
    expect(parser1.parse("(\\(x : Natural) -> f x + 1) 2") == Application(Lambda(VarName("x"), Constant(NanoConstant.Natural), Operator(Application(Variable(VarName("f"), 0), Variable(VarName("x"), 0)), NanoOperator.Plus, NaturalLiteral(1))), NaturalLiteral(2)))
  }
