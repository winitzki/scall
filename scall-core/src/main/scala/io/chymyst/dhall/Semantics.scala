...
object Semantics {
  ...
  private def betaNormalizeUncached(expr: Expression, stopExpanding: Boolean): BNResult = {
    ...
    expr.scheme match {
      ...
      case Lambda(name, tipe, body) => body.scheme match {
        // TODO: report issue, document the eta reduction rules in the Dhall standard.
        ...
        case _ => Lambda(name, bn(tipe), bnStopExpanding(body))
      }
