...
class ParserTest extends DhallTest {
  ...
  test("valid_non_ascii with large Unicode values from file") {
    ...
    val input: String = TestUtils.readToString((this.getClass.getResource("/valid_non_ascii.txt").toURI.getPath))
  ...
  test("builtin names") {
    val names = Builtin.namesToValuesMap
  ...
  test("constant names") {
    val names = Constant.namesToValuesMap
  ...
  test("identifier special cases") {
    check(Grammar.identifier(_), "Natural+blahblah", Expression(ExprBuiltin(Natural)), 7)
