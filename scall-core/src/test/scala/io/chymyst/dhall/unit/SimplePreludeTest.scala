...
import io.chymyst.dhall.unit.TestUtils.{DhallTest, UsingCaches, readToString}
...
class SimplePreludeTest extends DhallTest with TestTimeouts {
  ...
  test("import dhall-lang/Prelude/package.dhall without hanging") {
    setupEnvironment {
      enumerateResourceFiles("dhall-lang/Prelude", Some("package.dhall")).filter(_.getAbsolutePath contains "dhall-lang/Prelude/package.dhall").foreach {
        file =>
          val Parsed.Success(DhallFile(_, ourResult), _) = Parser.parseDhallStream(new FileInputStream(file))
          expect(ourResult.resolveImports(file.toPath).isInstanceOf[Expression])
          println(TestUtils.cacheStatistics)
