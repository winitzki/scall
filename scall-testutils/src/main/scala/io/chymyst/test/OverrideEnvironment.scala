...
trait OverrideEnvironment {
  ...
  /** Create a fake environment with some variables and run the code with that fake environment. Note: this will not catch any exceptions that the test code
    * might throw.
    *
    * @param vars
    *   The names and values of the environment variables that need to be changed.
    * @param testProgram
    *   The test code that needs to run in the fake environment.
    * @tparam R
    *   The type of test code's returned result value.
    * @return
    *   The result value computed by the test code.
    */
  ...
  /** Create a fake environment where some variables are _not_ present, and run the code with that fake environment. Note: this will not catch any exceptions
    * that the test code might throw.
    *
    * @param vars
    *   The names of the environment variables that need to be removed.
    * @param testProgram
    *   The test code that needs to run in the fake environment.
    * @tparam R
    *   The type of test code's returned result value.
